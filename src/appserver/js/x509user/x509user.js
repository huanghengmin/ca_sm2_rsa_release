                        ...
                        if (formPanel.form.isValid()) {
                            formPanel.getForm().submit({
                                ...
                                success: function () {
                                    ...
                                    var key_type = Ext.getCmp("modifyUser_x509User_key_type").getValue();
                                    // Ext.Ajax.request({
                        ...
                        Ext.Msg.confirm("警告", "是否重新签发证书,重发后原有证书不可用!", function (sid) {
                            if (sid == "yes") {
                                ...
                                if (csp == "Sonicom Beijing Crypto Provider v2.0") {
                                    ...
                                    var folder = fso.GetSpecialFolder(2);
                                    var cn = recode.get("cn");
                                    var province = recode.get("province");
                                    var city = recode.get("city");
                                    var o = recode.get("organization");
                                    var ou = recode.get("institution");
                                    ...
                                    if (csrData) {
                                        ...
                                        Ext.Ajax.request({
                                            ...
                                            timeout: 5 * 60 * 1000,
                                            method: "POST",
                                            // params: {csrData: Ext.util.base64.encode(csrData)},
                                            params: {csrData: csrData},
                                            success: function (res, action) {
                                                myMask.hide();
                                                var respText = Ext.util.JSON.decode(res.responseText);
                                                var url = respText.url;
